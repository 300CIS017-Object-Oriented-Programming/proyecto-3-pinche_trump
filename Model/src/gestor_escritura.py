class gestor_escritura:
    ...
    def guardar_excel(self, nombre_archivo, encabezados):
        try:
            self.archivo.columns = encabezados
    ...
    def guardar_json(self, nombre_archivo, encabezados):
        try:
            self.archivo.columns = encabezados
            self.archivo.to_json(nombre_archivo, index=False, engine="openpyxl")
        except Exception as e:
            raise ValueError(f"Error al guardar el archivo JSON: {e}")

    def guardar_csv(self, nombre_archivo, encabezados):
        try:
            self.archivo.columns = encabezados
